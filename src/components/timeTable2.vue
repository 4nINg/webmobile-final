...
export default {
  ...
  mounted() {
    ...
    console.log(this.totalMovieList);
  },
  methods: {
    next() {
        ...
        .then(response => {
          ...
          for (var q = 0; q < movieList.length; ++q) {
            ...
            for (var j = 0; j < byMovie.length; j++) {
              ...
              for (var k = 0; k < this.totalMovieList.length; ++k) {
                if (this.totalMovieList[k].includes(byTitleTime[0]) || byTitleTime[0].includes(this.totalMovieList[k])) {
                  flag = false;
                  break;
                }
              }
              if (flag) {
                this.totalMovieList.push(byTitleTime[0]);
              }
            }
            }catch(error){
              console.log(error);
